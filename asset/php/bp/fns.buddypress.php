...
function sc_filter_iframes($content) {     

    echo str_replace(['<iframe', '</iframe>'], ['<div class="iframe"><iframe', '</iframe></div>'], $content);
    
}; 
         
// add the filter 
add_filter('bp_get_activity_content_body', 'sc_filter_iframes', 10, 1); 

function sc_show_fav_count() {

    $fav_count = (bp_activity_get_meta(bp_get_activity_id(), 'favorite_count')) ? bp_activity_get_meta(bp_get_activity_id(), 'favorite_count') : 0;

    return '<span class="fav-count">'.$fav_count.'</span>';

}

add_action('bp_activity_entry_meta', 'sc_show_fav_count');
