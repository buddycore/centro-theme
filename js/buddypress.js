...
jq(document).ready( function() {
	...
	*/
	...
	jq('#aw-whats-new-submit').on( 'click', function() {
		...
		jq.post( ajaxurl, post_data, function( response ) {
			...
			if ( response[0] + response[1] === '-1' ) {
				...
				form.get(0).reset();
				// SC Edit
				jq('div.update-form').hide();
				jq('button.toggle-next-form').text('Post Another Update');
		...
		if ( target.hasClass('fav') || target.hasClass('unfav') ) {
			...
			function(response) {
				...
				if ( 'fav' === type ) {
					...
					// SC EDIT
					var newVal = parseInt(target.parent('li').parent('ul').parent('li').parent('ul').parent('div').prev('span').text()) + 1;
					target.parent('li').parent('ul').parent('li').parent('ul').parent('div').prev('span').text(newVal);
				...
				if ( 'activity-favorites' === jq( '.item-list-tabs li.selected').attr('id') ) {
					// SC EDIT
					target.closest( '.activity-item' ).hide(0);
				}
			});
		...
		if ( target.hasClass('delete-activity') ) {
			...
			function(response) {
				...
				if ( response[0] + response[1] === '-1' ) {
					...
					// SC Edit
					li.children('#message').hide(0).show(0);
				} else {
					// SC EDit
					li.hide(0);
		...
		if ( target.hasClass( 'spam-activity' ) ) {
			...
			function(response) {
				if ( response[0] + response[1] === '-1' ) {
					li.prepend( response.substr( 2, response.length ) );
					// SC Edit
					li.children( '#message' ).hide().show(0);
				} else {
					// SC EDIT
					li.hide(0);
					// reset vars to get newest activities
	...
	jq('div.activity').on('click', '.activity-read-more a', function(event) {
		...
		function(response) {
			// SC EDIT
			jq(a_inner).hide(0).html(response).show(0);
		});
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.attr('name') === 'ac_form_submit' ) {
			...
			jq.post( ajaxurl, ajaxdata, function(response) {
				...
				if ( response[0] + response[1] === '-1' ) {
					// SC EDIT
					form.prepend( jq( response.substr( 2, response.length ) ).hide().show(0) );
				} else {
					...
					form.fadeOut( 200, function() {
						...
						// SC EDIT
						activity_comments.children('ul').append( jq( the_comment ).hide().show(0) );
		...
		if ( target.hasClass('acomment-delete') ) {
			...
			function(response) {
				...
				if ( response[0] + response[1] === '-1' ) {
					// SC EDIT
					comment_li.prepend( jq( response.substr( 2, response.length ) ).hide().show(0) );
				} else {
		...
		if ( target.parent().hasClass('show-all') ) {
			...
			setTimeout( function() {
				// SC EDIT
				target.parent().parent().children('li').show(0, function() {
		...
		if ( target.hasClass( 'ac-reply-cancel' ) ) {
			// SC EDit
			jq(target).closest('.ac-form').hide(0);
			return false;
		}
	});
		...
		if ( keyCode === 27 ) {
			if (element.tagName === 'TEXTAREA') {
				if ( jq(element).hasClass('ac-input') ) {
					// SC EDIT
					jq(element).parent().parent().parent().hide(0);
				}
			}
		}
	});
